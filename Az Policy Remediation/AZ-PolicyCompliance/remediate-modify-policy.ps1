...
switch ($type) {
subscription {
$nonCompliantPolicies = Get-AzPolicyState -SubscriptionId $id | Where-Object { $_.ComplianceState -eq "NonCompliant" -and $_.PolicyDefinitionAction -eq "modify" -and $_.PolicyAssignmentScope -like "*subscriptions*" }
...
managementgroup {
$nonCompliantPolicies = Get-AzPolicyState -ManagementGroupName $id | Where-Object { $_.ComplianceState -eq "NonCompliant" -and $_.PolicyDefinitionAction -eq "modify" -and $_.PolicyAssignmentScope -like "*managementGroups*" }
foreach($policy in $nonCompliantPolicies)
{
Write-Output "Start remediation: $($policy.PolicyDefinitionName)"
$startremediation = Start-AzPolicyRemediation -Name "rem-$($policy.PolicyDefinitionName)" -PolicyAssignmentId $policy.PolicyAssignmentId -PolicyDefinitionReferenceId $policy.PolicyDefinitionId
...
resourcegroup {
$nonCompliantPolicies = Get-AzPolicyState -ResourceGroupName $id | Where-Object { $_.ComplianceState -eq "NonCompliant" -and $_.PolicyDefinitionAction -eq "modify" -and $_.PolicyAssignmentScope -like "*$id*" }
foreach($policy in $nonCompliantPolicies)
{
Write-Output "Start remediation: $($policy.PolicyDefinitionName)"
$startremediation = Start-AzPolicyRemediation -Name "rem-$($policy.PolicyDefinitionName)" -PolicyAssignmentId $policy.PolicyAssignmentId -PolicyDefinitionReferenceId $policy.PolicyDefinitionId
